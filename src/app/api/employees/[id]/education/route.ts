...
export async function POST(
  ...
  try {
    ...
    if (!isAdmin(user.roleId)) {
      const selfId = await getSelfEmployeeId(user.employeeUid);
      if (selfId !== employeeId) {
        return NextResponse.json({ success: false, error: 'Forbidden - You can only update your own education history' }, { status: 403 });
      }
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!isAdmin(user.roleId)) {
      const selfId = await getSelfEmployeeId(user.employeeUid);
      if (selfId !== employeeId) {
        return NextResponse.json({ success: false, error: 'Forbidden - You can only delete your own education history' }, { status: 403 });
      }
    }
