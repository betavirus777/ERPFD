            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="font-medium">{client.industryName || '-'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-1">Country</p>
                  <p className="font-medium">{client.countryName || '-'}</p>
                </div>
                <div>
              ...
              <div className="pt-4 border-t flex flex-wrap gap-4">
                {client.clientWebsite && (
                  <a 
                    href={client.clientWebsite.startsWith('http') ? client.clientWebsite : `https://${client.clientWebsite}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-blue-600 hover:underline"
                  >
                    <Globe className="w-4 h-4" /> {client.clientWebsite}
                    <ExternalLink className="w-3 h-3" />
                  </a>
                )}
                {client.contactNumber && (
                  <a href={`tel:${client.contactNumber}`} className="flex items-center gap-2 text-gray-600 hover:text-blue-600">
                    <Phone className="w-4 h-4" /> {client.contactNumber}
                  </a>
                )}
              </div>
            </CardContent>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{client.opportunities?.length || 0}</p>
              ...
              <CardContent>
                {client.projects && client.projects.length > 0 ? (
                  <Table>
                    ...
                    <TableBody>
                      {client.projects.map((project: any, idx: number) => (
                        <TableRow key={idx}>
                          ...
                          <TableCell>{project.endDate || project.end_date || '-'}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
              ...
              <CardContent>
                {client.opportunities && client.opportunities.length > 0 ? (
                  <Table>
                    ...
                    <TableBody>
                      {client.opportunities.map((opp: any, idx: number) => (
                        <TableRow key={idx}>
                          <TableCell className="font-medium">{opp.opportunityName || opp.opportunity_name}</TableCell>
                          <TableCell>${(opp.opportunityValue || opp.opportunity_total_cost || 0).toLocaleString()}</TableCell>
                          <TableCell>
                            <Badge variant="outline">{opp.statusName || 'Open'}</Badge>
                          </TableCell>
                          <TableCell>{opp.createdAt ? new Date(opp.createdAt).toLocaleDateString() : '-'}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
