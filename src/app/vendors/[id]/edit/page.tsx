...
import { vendorsAPI, mastersAPI } from '@/lib/api'
import {
  ArrowLeft,
  Save,
  Loader2,
...
export default function VendorEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const [vendorRes, countriesRes] = await Promise.all([
        vendorsAPI.getById(parseInt(id)),
      ...
      if (vendorRes?.data) {
        ...
        setFormData({
          ...
          vendor_website: vendor.vendorWebsite || vendor.vendor_website || '',
          contact_number: vendor.contactNumber || vendor.contact_number || '',
          vendor_address: vendor.vendorAddress || vendor.vendor_address || '',
          country_id: String(vendor.countryId || vendor.country_id || ''),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await vendorsAPI.update(parseInt(id), {
        ...formData,
      ...
      if (response.success) {
        ...
        setTimeout(() => router.push(`/vendors/${id}`), 1500)
      } else {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500">{formData.vendor_name}</p>
            </div>
          </div>
